...
public class ServerFrame extends JFrame implements ActionListener {
	...
	private final JButton jbt_start;
	private final JButton jbt_stop;
	...
	private final List<String> online_usernames;
	private final List<Integer> online_usernameids;
	public ServerFrame(Server server) {
		this.server = server;
		online_usernames = new ArrayList<>();
		online_usernameids = new ArrayList<>();
		try {
	...
	public void setDisUsers(String userNames) {
		if(userNames.equals("@ugliest")){
			jlt_disUsers.removeAll();
			String[] user_null = new String[]{};
			jlt_disUsers.setListData(user_null);
		}else{
			if(userNames.contains("@ugliest")){
				String[] dis = userNames.split("@userlist");
				String [] disUsername = new String[dis.length / 2];
				int j = 0;
				for(int i = 0; i < dis.length; i++){
					disUsername[j++] = dis[i++];
				}
				jlt_disUsers.removeAll();
				jlt_disUsers.setListData(disUsername);
			}
