...
public class Client_chatFrame extends JFrame implements ActionListener,
	...
	private final JTextField jtf_inputMess;
	private final JTextArea jta_disMess;
	private final JButton jbt_trans;
	private final JButton jbt_clear;
	private JButton jbt_exit;
	private final JList<String> jlt_disUsers;
	private final JButton jbt_singlChat;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jbt_singlChat) {
			String user_names = jlt_disUsers.getSelectedValue();
			if (user_names == null) {
				JOptionPane.showMessageDialog(this, "????????\\n???????????");
			} else {
	...
	public void setDisUsers(String chat_re) {
		...
		if (infos.length == 2) {
			...
			if (!client.c_singleFrames.isEmpty()) {
				ListModel<String> list = jlt_disUsers.getModel();
			...
			if ((infos.length / 2 - 1) < client.username_online.size()) {
				...
				for (int i = 0; i < client.username_online.size(); i++) {
					online.add(0, client.censused.get(i));
				}
				if (online.isEmpty()) {
					for (int i = 1; i < infos.length; i++) {
						...
						if (Integer.parseInt(infos[i]) != client.getThreadID()) {
							if (!online.contains(Integer.parseInt(infos[i]))) {
								...
								if (!name.equals(infos[i - 1])) {
									...
									client.censused.remove(new Integer(
											Integer.parseInt(infos[i])));
									client.username_online.add(0, infos[i - 1]);
									client.censused.add(0,
											Integer.parseInt(infos[i]));
	...
	private void createSingleChatFrame(String name) {
		Client_singleFrame c_singFrame = new Client_singleFrame(client, name);
		client.c_singleFrames.put(name, c_singFrame);
		c_singFrame.userThreadID = client.censused
				.get(client.username_online.indexOf(name));

		c_singFrame.setVisible(true);
	}

	public void setSingleFrame(String chat_re) throws Exception {
